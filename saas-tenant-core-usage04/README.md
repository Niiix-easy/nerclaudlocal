...
O módulo `/usage` implementa medição multi-tenant:

- Usage Events com `idempotencyKey` para evitar duplicidade;
- Meters com `SUM`, `COUNT`, `MAX` e `LAST`;
- Aggregation por período;
- Adjustments positivos ou negativos;
- Limits `SOFT` e `HARD`;
- Overage com unidades excedentes, preço unitário e valor;
- Tenant isolation em todas as consultas;
- API Key/JWT podem consumir a API conforme RBAC.

### Endpoints

- `GET /usage/meters`
- `POST /usage/meters`
- `PATCH /usage/meters/:meterId`
- `POST /usage/events`
- `GET /usage/events`
- `POST /usage/adjustments`
- `GET /usage/adjustments`
- `POST /usage/limits`
- `GET /usage/limits`
- `POST /usage/aggregate`
- `GET /usage/aggregation`
- `POST /usage/overage/calculate`
- `GET /usage/overage`
- `POST /usage/overage/:id/status`

### Exemplo de fluxo

1. Crie um meter `api_calls`, unidade `request`, aggregation `COUNT`.
2. Crie um limite mensal com `includedUnits` e `overageUnitPrice`.
3. Envie eventos com uma `idempotencyKey` única.
4. Agregue o período.
5. Calcule/persista o overage.

Para produção, a cobrança final deve ser integrada ao módulo de Billing/Invoices; o módulo de Usage é a fonte de consumo.
